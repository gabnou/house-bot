...
> **UI development:** run `npm run dev` inside `ui/` to start the Vite dev server at **http://localhost:5252** with HMR. It proxies `/admin/api/*` to the FastAPI server at `:8000`, so the bot must be running alongside. In production, always use **http://localhost:8000**.

### UI development

When working on the Control Panel source files (`ui/src/`), start the Vite dev server in a separate terminal for instant feedback via HMR:
...
The dev server is available at **http://localhost:5252** and proxies all `/admin/api/*` calls to the bot at `:8000`. When you're done, rebuild the production bundle:
